...
<style>
    ...
    .patient-card:hover { transform: translateY(-4px); box-shadow: var(--shadow-hover); }
    ...
    .badge-crisis  { background: rgba(244,67,54,0.1);  color:#f44336; border:1px solid rgba(244,67,54,0.2); }
    .badge-ok      { background: rgba(76,175,80,0.1);  color:#4caf50; border:1px solid rgba(76,175,80,0.2); }
    .badge-warning { background: rgba(255,152,0,0.1);  color:#ff9800; border:1px solid rgba(255,152,0,0.2); }
    .badge-neutral { background: rgba(90,124,122,0.1); color:var(--secondary); border:1px solid rgba(90,124,122,0.2); }
    ...
    .empty-state i { font-size: 4rem; opacity: 0.35; margin-bottom: 1.2rem; display: block; }
    .empty-state h3 { font-family: 'Quicksand', sans-serif; color: var(--primary-dark); margin-bottom: 0.5rem; }
</style>
@endpush
            ...
            <div class="patients-grid">
                @foreach ($pacientes as $p)
                    @php
                        ...
                        $fillClass  = $adherencia === null ? 'fill-orange'
                                    : ($adherencia >= 85 ? 'fill-green'
                                    : ($adherencia >= 60 ? 'fill-orange' : 'fill-red'));
                    @endphp
                    <a href="{{ route('especialista.pacientes.show', $p->id) }}" class="patient-card">
                        <div class="card-top">
                            <div class="avatar">{{ strtoupper(substr($p->name, 0, 2)) }}</div>
                            <div>
                        ...
                        <div class="card-badges">
                            @if ($p->crisis_flag)
                                <span class="badge badge-crisis"><i class="fas fa-exclamation-triangle"></i> Crisis detectada</span>
                            @else
                                <span class="badge badge-ok"><i class="fas fa-check-circle"></i> Sin alertas</span>
                            @endif

                            @if ($p->total_meds > 0)
                                <span class="badge badge-neutral"><i class="fas fa-pills"></i> {{ $p->total_meds }} medicamento(s)</span>
                            @endif
                        ...
                        @if ($adherencia !== null)
                            <div class="adherence-bar-wrap">
                                ...
                                <div class="progress-track">
                                    <div class="progress-fill {{ $fillClass }}" style="width: {{ $adherencia }}%;"></div>
                                </div>
                            </div>
                        @endif
